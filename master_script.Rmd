...
fit_lm_1_region <- function(train_data){ lm(votes_share ~ afinn_mean_daily + afinn_mean_region, data = train_data) }
...
lm_d_e <- train_data_elex_first_day %>%
  group_by(country) %>% 
  nest() %>%
  mutate(lm_1_d_e = purrr::map(data, ~fit_lm_1_day(.x)),
         lm_2_d_e = purrr::map(data, ~fit_lm_2_day(.x)),
         lm_3_d_e = purrr::map(data, ~fit_lm_3_day(.x))
  ) %>%
  ungroup() %>%
...
# Region
lm_r_e <- train_data_elex_first_region %>%
  group_by(country) %>% 
  nest() %>%
  mutate(lm_1_r_e = purrr::map(data, ~fit_lm_1_region(.x)),
         lm_2_r_e = purrr::map(data, ~fit_lm_2_region(.x)),
         lm_3_r_e = purrr::map(data, ~fit_lm_3_region(.x))
  ) %>%
  ungroup() %>%
  pivot_longer(cols = c(lm_1_r_e, lm_2_r_e, lm_3_r_e), names_to = "model_name", values_to = "model") %>%
...
fit_mr_4_day <- function(train_data){ lmer(votes_share ~ afinn_mean*n_likes_mean + (1 + afinn_mean | country),
                                           data = train_data) }


# Region
fit_mr_1_region <- function(train_data){
  lmer(votes_share ~ afinn_mean_daily + afinn_mean_region + phone + (1 | country), data = train_data)
}
...
fit_mr_4_region <- function(train_data){
  lmer(votes_share ~ afinn_mean_daily + afinn_mean_region + phone + (1 + afinn_mean_daily | country), data = train_data)
}

# Check training data
names(train_data_elex_first_day)
...
mr_d_e <- train_data_elex_first_day %>%
  ...
  mutate(mr_1_d_e = purrr::map(data, ~fit_mr_1_day(.x)),
         ...
         mr_4_d_e = purrr::map(data, ~fit_mr_4_day(.x))
  ) %>%
...
mr_r_e <- train_data_elex_first_region %>%
  ...
  mutate(mr_1_r_e = purrr::map(data, ~fit_mr_1_region(.x)),
         ...
         mr_4_r_e = purrr::map(data, ~fit_mr_4_region(.x))
...
mr_d_p <- train_data_polls_day %>%
  ...
  mutate(mr_1_d_p = purrr::map(data, ~fit_mr_1_day(.x)),
         ...
         mr_4_d_p = purrr::map(data, ~fit_mr_4_day(.x))
...
mr_r_p <- train_data_polls_region %>%
  ...
  mutate(mr_1_r_p = purrr::map(data, ~fit_mr_1_region(.x)),
         ...
         mr_4_r_p = purrr::map(data, ~fit_mr_4_region(.x)),
...
models_master <- bind_all_models(models_mr, lm_r_e, lm_d_e)
...
predictions_post_national <- prediction_master %>%
  ...
  group_by(country, leader, model_name, model_name_long) %>%
