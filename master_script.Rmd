...
mae_by_country <- left_join(mae_by_country,
                            supp %>%
                              group_by(country) %>%
                              slice_max(year, n = 1) %>%
                              select(-year),
...
mae_by_region %>%
  ...
  theme_devpublicopinion +
  labs(title = "Correlation between MAE and sample size (number of Tweets per day)",
       subtitle = "Mean absolute error (MAE) by sample size, measured as the average number of Tweets per day, for multilevel models",
       x = "Mean sample size (number of Tweets per day)",
       y = "MAE") +
