...
senti_per_day_elex <- join_election_to_tweets(senti_per_day, covariates)

# Select nearest election for each day
senti_per_day_elex <- select_nearest_election(senti_per_day_elex)

# Model 1
train_data_1 <- senti_per_day_elex %>%
  filter(year(date) %in% c(2014, 2015)) %>%
  filter(days_diff_abs < 10)

train_data_1$elex_date %>% unique # elections of training data
...
# Model 2
model_2 <- lmer(votes_share ~ n_tweets + (1 + afinn_mean|country),
                data = train_data_1)
...
```{r create-predictions-data}
# Create dataset of predictions
predictions <- senti_per_day_elex
...
# Model 1 ----
# Predict
test_data_1 <- senti_per_day_elex %>%
  filter(year(date) %in% c(2019)) %>%
  filter(days_diff_abs < 10)

predictions <- predictions %>%
  mutate(prediction_1 = predict(model_1, .))

# Calculate MAE
predictions %>%
  filter(year(date) %in% c(2019)) %>%
  filter(days_diff_abs < 10) %>%
  mutate(prediction_1_diff = prediction_1-votes_share,
         prediction_1_diff_abs = abs(prediction_1_diff)
  ) %>%
  summarise(mae = mean(prediction_1_diff_abs, na.rm = TRUE))
...
(n_elections_per_country <- tweets_df %>%
    group_by(leader_country, year(date)) %>%
    summarise(n = n()) %>%
    group_by(leader_country) %>%
    summarise(n = n()) %>%
...
# (n_regions_2_per_country <- tweets_df %>%
#     group_by(leader_country, region_2) %>%
#     summarise(n = n()) %>%
#     group_by(leader_country) %>%
#     summarise(n = n()) %>%
#     mutate(variable = "Number of regions_2 per country") %>%
#     # if n = 1, that means a country doesn't have any region_2
#     mutate(n = if_else(n == 1, as.integer(0), n)) %>%
#     pivot_wider(names_from = leader_country, values_from = n)
# )
