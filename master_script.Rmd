...
get_tweets_r(scrape_data, limit = 1000000, include_geocode = FALSE)

# Get tweets with points
get_tweets_r(scrape_data, limit = 1000000, include_geocode = TRUE)

```


```{r bind-and-save-tweets, eval = FALSE}
...
tweets_tokens_stem <- tweets_tokens %>%
...
word_lvls <- unique(c(tweets_tokens$word, afinn$word))

afinn <- afinn %>%
  transmute(word = factor(word, word_lvls),
            afinn_value = value)
...
senti_tweets <- tweets_tokens %>%
  as.data.frame() %>%
  as_tibble() %>%
  mutate(word = factor(word, word_lvls)) %>%
  left_join(.,
            afinn,
            by = "word")

# Share NA in lexicon join
table(is.na(senti_tweets$value)) %>% prop.table()
...
senti_mean <- senti_tweets %>%
  group_by(id, username, date, week, country, region_1, leader) %>%
  summarise(afinn_value = mean(afinn_value, na.rm = TRUE)) %>%
  # if sentiment is negative, binary is 0
  mutate(binary = if_else(afinn_value < 0, 0, 1)) %>%
  arrange(country, region_1, leader, date)


# Mean share of binary per day
senti_mean_day <- x
senti_mean %>%
  group_by(week, country, region_1, leader, binary) %>%
  summarise(binary_n = n()) %>%
  group_by(week, country, region_1, leader, )


summarise(afinn_value = mean(afinn_value, na.rm = TRUE)) %>%
  arrange(country, region_1, leader, date) %>%
  mutate(afinn_value_roll = roll_mean(afinn_value, n = 7)) %>%
  arrange(country, region_1, leader, date) %>%
  ungroup()
