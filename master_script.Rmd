...
estimates_mr_day <- create_test_data(senti_day_targets_covars) %>%
  mutate(prediction_day_elex_1 = predict(mr_day_elex_1, ., allow.new.levels = TRUE),
         prediction_day_elex_2 = predict(mr_day_elex_2, ., allow.new.levels = TRUE),
         prediction_day_polls_3 = predict(mr_day_elex_3, ., allow.new.levels = TRUE),
         prediction_day_polls_4 = predict(mr_day_elex_4, ., allow.new.levels = TRUE)
  )

estimates_mr_region <- create_test_data(senti_region_targets_covars) %>%
  mutate(prediction_region_elex_1 = predict(mr_region_elex_1, ., allow.new.levels = TRUE),
         prediction_region_elex_2 = predict(mr_region_elex_2, ., allow.new.levels = TRUE),
         prediction_region_polls_3 = predict(mr_region_elex_3, ., allow.new.levels = TRUE),
         prediction_region_polls_4 = predict(mr_region_elex_4, ., allow.new.levels = TRUE)
  )
...
estimates_mr_master <- estimates_mr_master %>%
...
mae_mr_by_model <- estimates_mr_master %>%
...
mae_mr_by_model <- bind_rows(mae_mr_by_model,
                             mae_mr_by_model %>%
...
ggplot(estimates_mr_master_sub,
  ...
  labs(title = "Model estimates against actual vote shares",
       subtitle = "Different model estimates (x-axis) against latest actual election vote shares (y-axis); diagonal line shows perfect predictions (error = 0)",
       x = "Estimate",
       y = "Votes share")
