...
prediction_master %>%
  filter(grepl("post", model_name_long)) %>% view
...
mae_by_country_plus %>%
  ...
  theme_devpublicopinion +
  labs(title = "Multilevel regression outperforms linear regression at all but one level across five countries",
       subtitle = "Average MAE of linear models, multilevel models, and the best performing MR 3 model,\nacross Zimbabwe, Georgia, Afghanistan, Mexico, and Nigeria. Note that MR 3 predictions at the national level used post-stratification.",
       x = NULL,
       y = "MAE, percentage points"
  ) +
  ggsave("output/figures/mae_by_country_and_model_type.png", height = 10, width = 14)
...
mae_by_country %>%
  ...
  #filter(country %in% c("Afghanistan", "Mexico")) %>%
  #filter(level == "Region") %>%
  mutate(mae = mae*100) %>%
...
mae_sample_table <- mae_sample_table %>%
  ...
  summarise(mae = mean(mae),
            n_tweets_daily_mean = mean(n_tweets_daily_mean, na.rm = TRUE)) %>%
  mutate(across(c(mae, n_tweets_daily_mean), ~round(., 2))) %>%
  rename("Model group" = model_group, "MAE" = mae, "Average number of Tweets per day" = n_tweets_daily_mean)

mae_sample_table

# Save as csv
write_csv(mae_sample_table, "output/tables/mae_sample_by_model_group.csv")
...
post_over_time %>%
  ...
  labs(title = "MRP estimates over time for developing country leaders and candidates",
       subtitle = "Estimated national favourability of a leader by multilevel regression with post-stratification (MRP);\nblack lines show 14-day rolling average of model predictions",
       x = NULL,
       y = "Estimated vote share") +
  ggsave("output/figures/estimates_over_time.png", height = 10, width = 14)
...
post_over_time %>%
  ...
  theme_devpublicopinion +
  labs(title = "MRP estimates over time for Zimbabwe",
       subtitle = "Estimated national favourability of a leader by multilevel regression with post-stratification (MRP);\nblack lines show 14-day rolling average of model predictions",
       x = NULL,
       y = "Estimated vote share") +
  ggsave("output/figures/estimates_over_time_zwe.png", height = 10, width = 14)
