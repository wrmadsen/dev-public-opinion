...
tweets_sub <- read_csv("data/tweets_sub.csv") 

senti_tokens <- read_csv("data/senti_tokens.csv")
...
(n_tweets_per_country <- tweets_sub %>%
...
(n_weekly_tweets_per_country <- tweets_sub %>%
    group_by(leader_country, week) %>%
    summarise(n = n()) %>%
    group_by(leader_country) %>%
    summarise(n = mean(n) %>% round(2)) %>%
    mutate(variable = "Average weekly number of tweets per country") %>%
    pivot_wider(names_from = leader_country, values_from = n)
)

# Number of unique weeks per country
(n_weeks_per_country <- tweets_sub %>%
    group_by(leader_country, week) %>%
    summarise(n = n()) %>%
    group_by(leader_country) %>%
    summarise(n = n()) %>%
...
# Years per country
(n_years_per_country <- tweets_sub %>%
...
(n_regions_1_per_country <- tweets_sf %>%
    ...
    group_by(leader_country) %>%
    summarise(n = n()) %>%
    mutate(variable = "Number of regions per country") %>%
    pivot_wider(names_from = leader_country, values_from = n)
)
