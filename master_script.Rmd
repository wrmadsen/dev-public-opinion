...
get_help %>%
  ...
  mutate(tweets = pmap(list(leader, geocode, date, date_plus_one),
                       ~get_tweets(..1,
                                   "en",
                                   ..2,
                                   20,
...
# Variables
hashtag_vars <- tweets_raw %>% names() %>% .[grepl("hashtags.", .)]
.vars <- rlang::syms(hashtag_vars)

sapply(tweets_raw %>% select(contains("hashtag")), class)
...
tweets_raw %>%
  mutate(across(where(matches("hashtags.")), ~if_else(is.na(.), "", paste(.))))
  transmute(hashtags_list = paste(!!!.vars, sep = ",", na.rm = TRUE)
  )
