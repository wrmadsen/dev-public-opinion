...
```{r linear-models}
# Linear models
fit_lm_1_day <- function(train_data){ lm(votes_share ~ afinn_mean, data = train_data) }
fit_lm_2_day <- function(train_data){ lm(votes_share ~ n_likes_mean, data = train_data) }
fit_lm_3_day <- function(train_data){ lm(votes_share ~ afinn_mean*n_likes_mean, data = train_data) }

fit_lm_1_region <- function(train_data){ lm(votes_share ~ afinn_mean_daily, data = train_data) }
fit_lm_2_region <- function(train_data){ lm(votes_share ~ n_likes_mean, data = train_data) }
fit_lm_3_region <- function(train_data){ lm(votes_share ~  afinn_mean_daily*n_likes_mean + afinn_mean_region,
                                            data = train_data) }
...
lm_r_e <- train_data_elex_first_raw %>%
  group_by(country) %>% 
  nest() %>%
  mutate(lm_1_r_e = purrr::map(data, ~fit_lm_1_day(.x)),
         lm_2_r_e = purrr::map(data, ~fit_lm_2_day(.x)),
         lm_3_r_e = purrr::map(data, ~fit_lm_3_day(.x))
  ) %>%
  ungroup() %>%
...
lm_a_e <- train_data_elex_first_adj %>%
  group_by(country) %>% 
  nest() %>%
  mutate(lm_1_a_e = purrr::map(data, ~fit_lm_1_region(.x)),
         lm_2_a_e = purrr::map(data, ~fit_lm_2_region(.x)),
         lm_3_a_e = purrr::map(data, ~fit_lm_3_region(.x))
  ) %>%
  ungroup() %>%
  pivot_longer(cols = starts_with("lm"), names_to = "model_name", values_to = "model") %>%
...
# Day
fit_mr_1_day <- function(train_data){
  
  lmer(votes_share ~ afinn_mean + (1 + afinn_mean | country), data = train_data)
  
}

fit_mr_2_day <- function(train_data){
  
  lmer(votes_share ~ n_likes_mean + (1 | country), data = train_data)
  
}

fit_mr_3_day <- function(train_data){
  
  lmer(votes_share ~ afinn_mean*n_likes_mean + (1 | country), data = train_data)
  
}

fit_mr_4_day <- function(train_data){
  
  lmer(votes_share ~ afinn_mean*n_likes_mean + (1 + afinn_mean | country), data = train_data)
...
fit_mr_1_region <- function(train_data){
  ...
  lmer(votes_share ~ afinn_mean_daily + phone + (1 + afinn_mean_daily | country), data = train_data)
  
}

fit_mr_2_region <- function(train_data){
  
  lmer(votes_share ~ n_likes_mean + phone + (1 | country), data = train_data)
  
}

fit_mr_3_region <- function(train_data){
  
  lmer(votes_share ~ afinn_mean_daily*n_likes_mean + afinn_mean_region + phone + (1 | country), data = train_data)
  
}

fit_mr_4_region <- function(train_data){
  
  lmer(votes_share ~ afinn_mean_daily*n_likes_mean + afinn_mean_region + phone + (1 + afinn_mean_daily | country), data = train_data)
...
mr_r_e <- train_data_elex_first_raw %>%
  ...
  mutate(mr_1_r_e = purrr::map(data, ~fit_mr_1_day(.x)),
         mr_2_r_e = purrr::map(data, ~fit_mr_2_day(.x)),
         mr_3_r_e = purrr::map(data, ~fit_mr_3_day(.x)),
         mr_4_r_e = purrr::map(data, ~fit_mr_4_day(.x))
  ) %>%
...
mr_a_e <- train_data_elex_first_adj %>%
  nest(data = everything()) %>%
  mutate(mr_1_a_e = purrr::map(data, ~fit_mr_1_region(.x)),
         mr_2_a_e = purrr::map(data, ~fit_mr_2_region(.x)),
         mr_3_a_e = purrr::map(data, ~fit_mr_3_region(.x)),
         mr_4_a_e = purrr::map(data, ~fit_mr_4_region(.x))
  ) %>%
  pivot_longer(cols = starts_with("mr"), names_to = "model_name", values_to = "model") %>%
  mutate(model_type = "adjusted")
...
mr_r_p <- train_data_polls_raw %>%
  nest(data = everything()) %>%
  mutate(mr_1_r_p = purrr::map(data, ~fit_mr_1_day(.x)),
         mr_2_r_p = purrr::map(data, ~fit_mr_2_day(.x)),
         mr_3_r_p = purrr::map(data, ~fit_mr_3_day(.x)),
         mr_4_r_p = purrr::map(data, ~fit_mr_4_day(.x))
  ) %>%
  pivot_longer(cols = starts_with("mr"), names_to = "model_name", values_to = "model") %>%
  mutate(model_type = "raw")
...
# Adjusted
mr_a_p <- train_data_polls_region %>%
  nest(data = everything()) %>%
  mutate(mr_1_a_p = purrr::map(data, ~fit_mr_1_region(.x)),
         mr_2_a_p = purrr::map(data, ~fit_mr_2_region(.x)),
         mr_3_a_p = purrr::map(data, ~fit_mr_3_region(.x)),
         mr_4_a_p = purrr::map(data, ~fit_mr_4_region(.x))
...
# Format into table
models_master$model[[1]]$obs


broom::glance(models_master$model[[1]])

# Linear
models_stats_lm <- models_master %>%
...
models_stats_lm %>%
  ...
  select(model_tidy, model_name_long) %>%
  unnest()
...
```{r create-test-data}
# Create test data
test_raw <- create_test_data(senti_day_targets_covars, choose_type = "election") %>%
  mutate(test_type = "raw")

test_adjusted <- create_test_data(senti_region_targets_covars, choose_type = "election") %>%
...
# Average national MAE for multilevel models with or withour post-stratification
mae_by_model %>%
  filter(level == "National") %>%
...
model_to_plot <- models_master %>%
  filter(model_name_long == "MR 4, election fitted, day") %>%
  .$model
...
names(ranef_labs) <- c("(Intercept)", "afinn_mean")
...
ggplot(re_to_plot,
  ...
  facet_wrap(~ term,
             ...
             scales = "free_x") +
  theme_devpublicopinion +
  labs(title = "Random effects of intercept and country-varying sentiment",
       subtitle = "Random effects associated with varying intercepts and varying slopes of AFINN sentiment; note that scales differ by facet",
