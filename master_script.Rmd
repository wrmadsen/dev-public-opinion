...
senti_targets_all <- join_targets(senti_day, targets_master)
...
# Add GDL covariates
senti_targets_covars <- add_gdl_covariates(senti_targets, gdl_interpo)

# Check which regions are missing
senti_targets_all$region_1 %>% unique
senti_targets$region_1 %>% unique


```


```{r create-predictions-object}
...
# N per country
train_data_1 %>% group_by(country) %>% summarise(n = n())

# No RE
model_1_1 <- lmer(votes_share ~ afinn_mean + (1 | country_and_region_1),
                  data = train_data_1)

# No RE, sentiment and structure
model_1_2 <- lmer(votes_share ~ afinn_mean + days_diff_abs + (1 | country_and_region_1),
                  data = train_data_1)

# RE of sentiment
model_1_3 <- lmer(votes_share ~ afinn_mean + (1 + afinn_mean | country_and_region_1),
                  data = train_data_1)

# RE, structure and days difference
model_1_4 <- lmer(votes_share ~ afinn_mean + n_likes_mean + days_diff_abs + (1 + afinn_mean | country_and_region_1),
                  data = train_data_1)
...
```{r elex-model-3}
# Fit model 3 based on election results
