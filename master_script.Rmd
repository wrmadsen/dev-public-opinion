...
twitter_mau_raw <- read_csv("data/raw/twitter_mau.csv")
...
supp <- cpi %>%
  ...
  left_join(twitter_mau, by = c("country")) %>%
mutate(eng_prop = eng_total/pop_total,
       gdp_ppp_pc = gdp_ppp/pop_total,
       mau_2020_pc = mau_2020/pop_total
) %>%
...
supp %>%
  ...
  ggplot(.,
         aes(x = wgi_est,
             y = eng_prop)) +
  geom_point(aes(size = mau_2020_pc)) +
