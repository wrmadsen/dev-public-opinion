...
mr_day_elex_4 <- lmer(votes_share ~ afinn_mean + (1 + afinn_mean + days_diff_abs | country),
                      data = train_data_elex_first_day)
...
# Fit region models
# By election results
mr_region_elex_1 <- lmer(votes_share ~ afinn_mean_daily + (1 | country),
                         data = train_data_elex_first_region)

mr_region_elex_2 <- lmer(votes_share ~ afinn_mean_daily + afinn_mean_region + (1 + afinn_mean_daily | country),
                         data = train_data_elex_first_region)

mr_region_elex_3 <- lmer(votes_share ~ afinn_mean_daily + n_likes_mean + (1 + afinn_mean_daily | country),
                         data = train_data_elex_first_region)

mr_region_elex_4 <- lmer(votes_share ~ afinn_mean_daily + days_diff_abs + (1 + afinn_mean_daily | country),
                         data = train_data_elex_first_region)

# Trained on polls
mr_region_polls_1 <- lmer(votes_share ~ afinn_mean_daily + (1 | country),
                          data = train_data_polls_region)

mr_region_polls_2 <- lmer(votes_share ~ afinn_mean_daily + (1 + afinn_mean_daily | country),
                          data = train_data_polls_region)
...
```{r predict-mr}
# Predict model 1
# predictions_1 <- create_test_data() %>%
...
# Find MR model estimates
estimates_mr_day <- create_test_data(senti_day_targets_covars) %>%
  mutate(prediction_mr_day_elex_1 = predict(mr_day_elex_1, ., allow.new.levels = TRUE),
         prediction_mr_day_elex_2 = predict(mr_day_elex_2, ., allow.new.levels = TRUE),
         prediction_mr_day_polls_3 = predict(mr_day_elex_3, ., allow.new.levels = TRUE),
         prediction_mr_day_polls_4 = predict(mr_day_elex_4, ., allow.new.levels = TRUE)
  )

estimates_mr_region <- create_test_data(senti_region_targets_covars) %>%
  mutate(prediction_mr_region_elex_1 = predict(mr_region_elex_1, ., allow.new.levels = TRUE),
         prediction_mr_region_elex_2 = predict(mr_region_elex_2, ., allow.new.levels = TRUE),
         prediction_mr_region_polls_3 = predict(mr_region_elex_3, ., allow.new.levels = TRUE),
         prediction_mr_region_polls_4 = predict(mr_region_elex_4, ., allow.new.levels = TRUE)
  )

# Bind estimates
estimates_mr_master <- bind_rows(estimates_mr_day,
                                 estimates_mr_region) %>%
  select(date_target, country, region_1, region_2, leader, popshare, eye, phone, cellphone,
         days_diff_abs, days_diff,
         starts_with("prediction"), votes_share)
...
estimates_mrp_master <- estimates_mr_master %>%
  mutate(across(starts_with("prediction"), ~.*popshare, .names = "{.col}_post"))

```

# Estimates master
```{r pivot-estimates-master}
# Pivot estimates master
# Consists of MR, MRP, and raw estimates
estimates_master <- estimates_mrp_master %>%
  pivot_longer(cols = c(starts_with("prediction")), values_to = "estimate", names_to = "model") %>%
  filter(!is.na(estimate)) %>%
  # Calculate error
  mutate(error = estimate - votes_share,
         error_abs = abs(error))

# Unique models
estimates_master$model %>% unique()
...
mae_mr_by_model_country <- estimates_mr_master %>%
...
mae_mr_by_model_country <- bind_rows(mae_mr_by_model_country,
                                     mae_mr_by_model_country %>%
...
mae_mr_by_model_region <- estimates_mr_master %>%
  filter(days_diff_abs < 10) %>%
  group_by(model, country, region_1, region_2) %>%
...
(mae_mr_by_model_region <- bind_rows(mae_mr_by_model_region,
                                     mae_mr_by_model_region %>%
                                       mutate(model = "Model average") %>%
                                       group_by(model, country, region_1, region_2) %>%
                                       summarise(votes_share = mean(votes_share, na.rm = TRUE),
                                                 estimate = mean(estimate, na.rm = TRUE),
...
mae_mr_by_model_country %>%
  mutate(model = as.factor(model),
         country = reorder_within(country, mae, model)) %>%
  ggplot(aes(x = country, y = mae, fill = model)) +
...
estimates_mr_master_sub <- estimates_mr_master %>%
  filter(days_diff_abs < 15)

ggplot(estimates_mr_master_sub,
       aes(x = estimate,
           y = votes_share)) +
  ...
  theme_devpublicopinion +
  labs(title = "Model estimates against actual vote shares",
       subtitle = "Different model estimates (x-axis) against latest actual election vote shares (y-axis); diagonal line shows perfect predictions (error equals 0)",
       x = "Estimate",
       y = "Votes share")


# Plot average predictions by region as points
ggplot(mae_mr_by_model_region,
       aes(x = estimate,
           y = votes_share)) +
  geom_point(aes(colour = country),
             size = 3,
  ...
  scale_colour_discrete(name = "") +
  theme_devpublicopinion +
  labs(title = "Average model estimates against actual vote shares by region",
...
estimates_master_num <- estimates_master %>%
  select(where(is.numeric))

# Calculate correlation
estimates_master_cor <- estimates_master_num %>%
  cor()

#cormat <- round(cor(mat), 2)

estimates_master_cor <- estimates_master_cor %>%
  as_data_frame() %>%
  mutate(x = colnames(estimates_master_num)) %>%
  pivot_longer(names_to = "y", values_to = "value", popshare:error_abs)

estimates_master_cor %>%
    arrange(x, y) %>%
